...
CCustomDamageInd::CCustomDamageInd()
{
	m_NumItems = 0;
}

void CCustomDamageInd::Create(vec2 Pos, vec2 Dir, float Alpha)
{
	if(m_NumItems >= MAX_ITEMS)
		return;

	if(m_LastAddTime == time_get())
	{
		return;
	}
	...
	pItem->m_Pos = Pos;
	pItem->m_Dir = -Dir;
	if(g_Config.m_ClDamageIndicatorEffect == 1)
		pItem->m_RemainingLife = 1.0f;
	else if(g_Config.m_ClDamageIndicatorEffect == 2)
	{
		pItem->m_Dir = direction(angle(-Dir) + 0.75f);
		pItem->m_RemainingLife = 1.05f;
	}

	pItem->m_StartAngle = -random_angle();
	pItem->m_Color = ColorRGBA(1.0f, 1.0f, 1.0f, Alpha);
	++m_NumItems;
	m_LastAddTime = time_get();
}

void CCustomDamageInd::OnRender()
{
	...
	for(int i = 0; i < m_NumItems;)
	{
		...
		else if(g_Config.m_ClDamageIndicatorEffect == 1)
		{
		...
		else if(g_Config.m_ClDamageIndicatorEffect == 2)
		{
			...
			for(int j = 0; j < Amount; j++)
			{
				...
				vec2 Pos = mix(m_aItems[i].m_Pos + (m_aItems[i].m_Dir + direction(j * 270.0f / Amount)) * 25.0f, m_aItems[i].m_Pos, std::clamp((m_aItems[i].m_RemainingLife - 0.95f) / 0.15f, 0.0f, 1.0f));
				const float LifeAlpha = m_aItems[i].m_RemainingLife / 0.1f;
				Graphics()->SetColor(m_aItems[i].m_Color.WithMultipliedAlpha(LifeAlpha));
				Graphics()->QuadsSetRotation(m_aItems[i].m_StartAngle + m_aItems[i].m_RemainingLife * 2.0f);
				Graphics()->RenderQuadContainerAsSprite(m_DmgIndQuadContainerIndex, 0, Pos.x, Pos.y);
